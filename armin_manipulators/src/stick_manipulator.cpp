...
void fillXYZfromDATA_POS_message(const char *data, int &x, int &y, int&z)
{
    ...
    y = received.tenzor[1];
    z = received.tenzor[2];
	ROS_INFO_STREAM("after message received x="<<x<<"  y="<<y<<"   z="<<z<<"    zOri="<<received.sensor_z);
}
...
double vectorX(double x)
{
    if (x >=720 && x <= 940)
        return 0;
    if (x > 940)
        return (x - 940)/(1200. - 940.);

    if (x < 720)
        return (x - 720.)/(720.-470.);

    return 0;
}

double vectorY(double y)
{
    if (y >=2430 && y <= 2690)
        return 0;
    if (y > 2690)
        return (y - 2690.)/(2860. - 2690.);

    if (y < 2430)
        return (y - 2430.)/(2430. - 2290);

    return 0;
}

double vectorZ(double z)
{
    if (z >=2200 && z <= 2450)
        return 0;
    if (z >2450)
        return (z - 2450.)/(3790. - 2450.);

    if (z < 2200)
        return (z - 2200.)/(2200. - 860.);
...
int read_event(int fd, struct axis_state axes[3], struct data & d_ptr)
{
   ...
   d_ptr.button &= ~(BUTTONS_USER_PRESENCE);
    ...
    if (rv) {
        ...
        auto xNew = vectorX(x);  // +- 200 and 0 as a result
        auto yNew = vectorY(y);
        auto zNew = vectorZ(z);
...
ROS_INFO_STREAM("setting  X="<<axes[1].y<<"   Y="<<axes[0].x<<"   Z="<<axes[0].y<<"    Z-rotation="<<d_ptr.sensor_z);
ROS_INFO_STREAM("will transform to dX="<< (JointStatesKeeper::LINEAR_VELOCITY_MAXIMAL/32000 * axes[1].y)<<"   dY="<<(JointStatesKeeper::LINEAR_VELOCITY_MAXIMAL*axes[0].x/32000)<<"   dZ="<<(JointStatesKeeper::LINEAR_VELOCITY_MAXIMAL* axes[0].y / 32000)); 
...
signal(SIGINT, mySigintHandler);
    ...
    int fd = open("/dev/ttyACM0", O_RDWR | O_NDELAY);
    if (fd < 0) {
        perror("Opening ttyACM0: ");
        ROS_ERROR("failed to open sphere device /dev/ttyACM0. Exiting");
        exit(0);
    } else {
        setUpSerial(fd);
    }
	...
	bool readingSuccessful = (read_event(fd, axes, lastFetchedData) == 0);
        if (readingSuccessful) {
                    sc.updateButtons(6, IS_ONLY_ROTATION_FORCED(lastFetchedData));
                    sc.updateButtons(1,IGNORE_ORIENTATION_CLICKED(lastFetchedData));
                sc.updateAxesState(axes);
                sc.updateSensorsAndRest(lastFetchedData, true);
        }
        if ( sc.isInitialized() && IS_USER_PRESENT(lastFetchedData))
        {
            sc.publishSphereJointsCommand(chatter_pub, true);
        }
	...
	if (readingSuccessful)
	{
        ...
        // as a matter of fact, this should be also guarded by
        if (IS_USER_PRESENT(lastFetchedData))
        {
            sc.setOpenRq(IS_BUTTON_OPEN(lastFetchedData));
            sc.setCloseRq(IS_BUTTON_CLOSE(lastFetchedData));
...
//		std::cout<<"Sending values x="<<message.x<<"   y="<<message.y<<"   z="<<message.z<<std::endl;
		...
		v = write(fd, &message, sizeof(message));
                if (v!= sizeof(message))
                 {
                    //std::cout<<"send only "<<v<<" bytes "<<"out of "<<sizeof(message)<<"  required"<<std::endl;
                }
		lastSendMoment = thisMoment;
            }
        }
